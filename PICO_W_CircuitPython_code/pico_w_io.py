...
import time  # ______________________________________________ we use time.monotonic aka seconds in float, to control the loop and NO time.sleep() any more..

from pid import PID # _______________________________________ file pid.py

DIAG = True  # False
DIAG = bool(os.getenv('DIAG')) # ____________________________ now get from settings.toml
...
# ___________________________________________________________ PICOW_IO

led = digitalio.DigitalInOut(board.LED)
led.direction = digitalio.Direction.OUTPUT
led.value = True  # _________________________________________ after boot LED ON helps to see its working..
dp("___ +++ board LED ON")

# ________________________________________________________ old config for GP1 ON OFF
#DO1set = False
#DO1 = digitalio.DigitalInOut(board.GP1)
#DO1.switch_to_output(value=True) # ______________________ no boot blink! thanks @deʃhipu
#DO1.direction = digitalio.Direction.OUTPUT
#DO1.value = not DO1set  # _______________________________ after boot DO1 ( ext LED output high ) aka LED OFF BUT IT BLINKS
#dp("___ +++ DO1 on GP1 HIGH >> LED OFF")
# ________________________________________________________ new config for GP1 PWM to ext LED ( or a servo for real process control )
DO1val = float(100)
...
DO1set(DO1val)
dp("___ +++ DO1 on GP1 pin 2 use as PWM 'analog' out, now 100.0pct full on")
...
RESET=0.01
RATE=0.1
pid=PID(GAIN,RESET,RATE,setpoint=SP) # __________ init control
pid.auto_mode
pid.output_limits = (0,100) # incl internal "anti-reset-wind-up" integral limit
...
def getAins(): # _________________________________________ changed to use PCT
...
def runPID():
    ...
    PV=AI0val # __________________________________________ link hardware Ain 0 to PV of control
    if pid.auto_mode :
        OUT = pid(PV) # __________________________________ call pid and get its reaction
    DO1val = OUT
    DO1set(DO1val) # _____________________________________ drive the hardware PWM output
    pids= f" PV: {PV:,.2f} [pct], SP: {SP:,.2f} [pct], OUT: {OUT:,.2f} [pct]"
    #dp(pids)


def get_datas(): # ________________________________________ to code.py to run & get the measurements
    return datas
...
def get_OUT(): # ___________________________________________ to web_wifi.py
    return OUT
