...
void mypid_setup(double PV, double SP) { // _________________________ initialize the variables we're linked to
  ...
  Setpoint = SP;
  myPID.SetMode(1); // AUTOMATIC ______________________________________ turn the PID on
}
...
void mypid_setMode(int Mode = 2, double OUT_MAN = 0.0) { // my MODEs : 0 LOCK, 1 MAN, 2 AUTO, 3 CASC
  ...
  if ( Mode == 1 ) { // MAN
    myPID.SetMode(0); // MANUAL ______________________________________ turn the PID off
...
double mypid_loop(double PV, double SP, double OUT_MAN, int Mode) {
  Input = PV;
  mypid_setMode(Mode, OUT_MAN); // ________________________________ this overwrite my remote setting
  myPID.Compute();
