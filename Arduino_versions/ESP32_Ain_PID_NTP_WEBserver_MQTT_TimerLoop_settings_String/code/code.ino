/*
  ...
  rev from ESP32_Ain_PID_NTP_WEBserver_MQTT_TimerLoop_settings to use STRING and less CHAR's
...
void setup(void) {
  ...
  set_IO(); // ____________________________________________________ in esp32_io.ino
...
void loop(void) {
  ...
  loop1min();
  loopM();
  // moved to the 1 sec job in loopC ! run_WIFI(); // ____________________________________________________ see web_wifi.ino
}
